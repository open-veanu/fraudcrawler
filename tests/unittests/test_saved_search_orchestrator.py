...
def _saved_source(name: str) -> WebsiteSource:
    return build_website_source_profile(
        name=name,
...
@pytest.mark.asyncio
async def test_run_accepts_legacy_saved_search_sources_keyword(monkeypatch):
    orchestrator = _DummyOrchestrator(
        searcher=object(),  # type: ignore[arg-type]
        enricher=object(),  # type: ignore[arg-type]
        url_collector=object(),  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_srch_execute_propagates_website_source_metadata():
    class _DummySearcher:
        async def capply(self, **kwargs):
            return [
                SearchResult(
                    url="https://shop.test/p/123",
                    domain="shop.test",
                    search_engine_name="boost_shop_search_engine",
                    website_source=WebsiteSourceMetadata(
                        source_name="Boost Shop",
                        resolved_url="https://shop.test/search?q=fridge",
    ...
    orchestrator = _DummyOrchestrator(
        searcher=_DummySearcher(),  # type: ignore[arg-type]
        enricher=object(),  # type: ignore[arg-type]
        url_collector=object(),  # type: ignore[arg-type]
        zyteapi=object(),  # type: ignore[arg-type]
        processor=object(),  # type: ignore[arg-type]
    )
    queue_in: asyncio.Queue[dict | None] = asyncio.Queue()
    queue_out: asyncio.Queue[ProductItem | None] = asyncio.Queue()
    await queue_in.put(
        {
            "search_term": "fridge",
            "search_term_type": "initial",
            ...
            "marketplaces": None,
            "excluded_urls": None,
            "website_source_source": _saved_source("Boost Shop"),
        }
    )
    await queue_in.put(None)

    await orchestrator._srch_execute(queue_in=queue_in, queue_out=queue_out)

    product = await queue_out.get()
    assert product is not None
    assert product.website_source is not None
    assert product.website_source.source_name == "Boost Shop"
    assert product.website_source.render_http_status == 200

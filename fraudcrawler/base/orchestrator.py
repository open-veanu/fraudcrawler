...
class Orchestrator(ABC):
            ...
            try:
                ...
                for res in results:
                    product = ProductItem(
                        ...
                        domain=res.domain,
                        product_name=res.candidate_title,
                        product_price=res.candidate_price,
                        product_images=res.candidate_images,
                        filtered=res.filtered,
                        filtered_at_stage=res.filtered_at_stage,
                        website_source=res.website_source,
                    )
    ...
    async def _add_srch_items(
        ...
        website_source_sources: List[WebsiteSource] | None,
        language: Language,
        location: Location,
        deepness: Deepness,
        marketplaces: List[Host] | None,
        ...
        async def _add_initial_items_for_engine(
            ...
            if search_engine != SearchEngineName.WEBSITE_SOURCE:
            ...
            sources = website_source_sources or []
            if not sources:
                logger.warning(
                    "Search engine '%s' selected without website_source_sources; skipping.",
                    SearchEngineName.WEBSITE_SOURCE.value,
                )
                return
            ...
            for source in sources:
                ...
                logger.debug(f'Adding website-source item="{item}" to srch_queue')
                await queue.put(item)
        ...
        if enrichment:
            ...
            for trm in terms:
                for se in search_engines:
                    if se == SearchEngineName.WEBSITE_SOURCE:
                        continue
                    await self._add_search_items_for_search_term(
                        search_term=trm,
    ...
    async def run(
        ...
        saved_search_sources: List[WebsiteSource] | None = None,
    ) -> None:
        ...
        """
        if saved_search_sources is not None:
            if website_source_sources is not None:
                raise ValueError(
                    "Provide only one of website_source_sources or saved_search_sources."
                )
            logger.warning(
                "saved_search_sources is deprecated; use website_source_sources instead."
            )
            website_source_sources = saved_search_sources
        ...
        if (
            ...
            and SearchEngineName.WEBSITE_SOURCE not in search_engines
        ):
            raise ValueError(
        ...
        if previously_collected_urls:
            await self._url_collector.add_previously_collected_urls(
        ...
        n_non_saved_engines = sum(
            1 for engine in search_engines if engine != SearchEngineName.WEBSITE_SOURCE
        )
        n_saved_engine_jobs = (
            n_saved_sources if SearchEngineName.WEBSITE_SOURCE in search_engines else 0
        )
        n_enriched_terms = (
